...
- 当针对一个**需求**进行提问时，可能提出多个问题和它们的子问题，必须使用 todo 工具来记录这些**问题**以及它们的层级关系。一次只处理一个**问题**，必须通过多轮对话的方式获得用户针对该**问题**的完整意图。用户必须**明确**回答所有问题，不要认为用户会接受你的任何建议。当针对这个**需求**的所有**问题**都回答完毕，才能处理下一个**需求**。
