...
### 用户故事3：版本升级管理 (优先级 P1)
作为基础设施团队，我需要支持扩展版本升级（如Redis扩展v1.0到v2.0）和中间件版本升级（如Redis 6.0到7.0），且在升级前后通过生命周期API执行准备、验证和收尾工作，不影响现有集群运行。

**优先级原因**：扩展版本升级支持平台功能演进，中间件版本升级支持软件特性更新，两者都需要通过生命周期API确保升级过程安全可控。

**独立测试**：可以通过扩展版本升级或中间件版本升级完成测试，验证生命周期API在升级前后被正确调用。

**验收场景**：
1. **给定** 平台已安装Redis扩展v1.0（支持Redis 6.0），且有运行中的Redis 6.0集群，**当** 安装Redis扩展v2.0并执行扩展版本升级操作，**那么** 系统调用pre-upgrade API执行准备工作，集群滚动重启完成升级，期间调用post-upgrade API验证升级结果，服务保持可用，扩展版本升级到v2.0
2. **给定** 平台已安装Redis扩展v2.0（支持Redis 6.0和7.0），且有运行中的Redis 6.0集群，**当** 用户提交中间件版本升级到Redis 7.0的请求，**那么** 系统调用pre-upgrade API执行兼容性检查和数据备份，按滚动顺序升级各节点，调用post-upgrade API验证集群功能正常，期间服务可用
3. **给定** 升级过程中出现问题，**当** 用户执行回滚操作，**那么** 系统调用pre-upgrade-rollback API准备回滚环境，执行回滚操作，调用post-upgrade-rollback API验证回滚结果，集群恢复到升级前状态
...
- 当pre-upgrade生命周期API返回不兼容或备份未完成的响应时，系统暂停升级流程，等待用户处理
- 当中间件版本升级跨多个版本（如6.0→7.0）且扩展声明需要分步升级时，系统必须按声明的路径（6.0→6.2→7.0）自动执行多步升级
...
- **FR-005**: 系统必须提供专用工作负载 ExPodSet，支持指定缩容时删除的Pod编号
...
- **FR-009**: 系统必须定义通用生命周期REST API接口，扩展可选择性实现；其中升级相关接口（pre-upgrade、post-upgrade、pre-upgrade-rollback、post-upgrade-rollback）适用于扩展版本升级和中间件版本升级两种场景，扩展通过实现这些接口来执行升级前的准备、升级后的验证以及升级失败的回滚操作
...
- **FR-014**: 系统必须支持通过REST API对单个Pod执行进程重启操作，由进程管理器执行停止后启动，新进程获得新PID，Pod和容器不重启
- **FR-015**: 系统必须支持进程级健康检查，当中间件进程停止时，Pod的Readiness Probe应自动变为Not Ready，从Service端点移除
- **FR-016**: 系统必须支持批量进程操作，用户可对整个集群或指定分组（如所有从节点）执行启动/停止/重启操作，支持配置并发数和滚动间隔策略
- **FR-017**: 进程管理器必须支持自动拉起模式（可配置），当中间件进程异常退出（非用户主动停止）时，进程管理器自动重新启动进程
- **FR-018**: 进程管理器必须支持配置热更新，用户可修改进程管理器配置（如自动拉起策略、健康检查参数）而无需重启Pod
- **FR-019**: 进程管理器作为PID 1运行时，当收到SIGTERM停止信号，必须将信号转发给中间件进程，等待其优雅退出后，进程管理器自身才退出
- **FR-020**: 系统必须支持自动备份，备份策略由用户配置，扩展定义备份步骤，备份存储于对象存储
- **FR-021**: 系统必须支持跨集群恢复，只要多集群能访问相同备份仓库
- **FR-022**: 系统必须支持配置热更新，通过Cue模板验证配置，扩展提供配置模板，用户提供完整配置文件
- **FR-023**: 系统必须支持配置回滚到上一版本
- **FR-024**: 系统必须通过Prometheus格式暴露监控指标，扩展在Pod模板中提供exporter sidecar定义
- **FR-025**: 系统必须支持多种日志收集方式，包括stdout输出、文件日志、日志轮转，由扩展通过sidecar实现
- **FR-026**: 系统必须集成cert-manager为中间件提供TLS证书和自动轮转能力
- **FR-027**: 系统必须支持私有镜像仓库，并支持配置镜像拉取密钥
- **FR-028**: 系统必须使用Kubernetes原生RBAC控制权限，用户的操作权限完全由其K8s权限决定
- **FR-029**: 中间件Pod必须自动探测并更新自身角色Label
- **FR-030**: 系统必须支持中间件版本升级，扩展声明支持的升级路径（如6.0→6.2→7.0），系统根据升级路径执行多步升级
- **FR-031**: 系统在执行中间件版本升级前，必须通过pre-upgrade生命周期API执行兼容性检查和数据备份确认
- **FR-032**: 系统在执行中间件版本升级后，必须通过post-upgrade生命周期API验证集群功能正常，验证失败支持自动或手动回滚
