...
- 提供专用工作负载（ExPodSet），具备标准StatefulSet的所有能力（有序部署、持久化存储、网络标识稳定等），同时支持高级特性（指定缩容时删除的Pod编号、自定义滚动重启顺序、细粒度的Pod生命周期控制）
- 通过静态/动态分组抽象支持任意部署拓扑
...
- **FR-008**: 系统必须支持静态分组和动态分组的拓扑抽象，静态分组定义Pod模板，动态分组管理具体副本
...
- **FR-016**: 系统必须支持进程级健康检查，当中间件进程停止时，Pod的Readiness Probe应自动变为Not Ready，从Service端点移除
- **FR-017**: 系统必须支持批量进程操作，用户可对整个集群或指定分组（如所有从节点）执行启动/停止/重启操作，支持配置并发数和滚动间隔策略
- **FR-018**: 进程管理器必须支持自动拉起模式（可配置），当中间件进程异常退出（非用户主动停止）时，进程管理器自动重新启动进程
- **FR-019**: 进程管理器必须支持配置热更新，用户可修改进程管理器配置（如自动拉起策略、健康检查参数）而无需重启Pod
- **FR-020**: 进程管理器作为PID 1运行时，当收到SIGTERM停止信号，必须将信号转发给中间件进程，等待其优雅退出后，进程管理器自身才退出
- **FR-021**: 系统必须支持自动备份，备份策略由用户配置，扩展定义备份步骤，备份存储于对象存储
- **FR-022**: 系统必须支持跨集群恢复，只要多集群能访问相同备份仓库
- **FR-023**: 系统必须支持配置热更新，通过Cue模板验证配置，扩展提供配置模板，用户提供完整配置文件
- **FR-024**: 系统必须支持配置回滚到上一版本
- **FR-025**: 系统必须通过Prometheus格式暴露监控指标，扩展在Pod模板中提供exporter sidecar定义
- **FR-026**: 系统必须支持多种日志收集方式，包括stdout输出、文件日志、日志轮转，由扩展通过sidecar实现
- **FR-027**: 系统必须集成cert-manager为中间件提供TLS证书和自动轮转能力
- **FR-028**: 系统必须支持私有镜像仓库，并支持配置镜像拉取密钥
- **FR-029**: 系统必须使用Kubernetes原生RBAC控制权限，用户的操作权限完全由其K8s权限决定
- **FR-030**: 中间件Pod必须自动探测并更新自身角色Label
- **FR-031**: 平台必须提供通用生命周期API Server镜像，扩展基于此镜像制作专用镜像，只需提供配置文件声明API与脚本的映射关系，无需编写HTTP Server代码
- **FR-032**: 通用生命周期API Server必须通过配置文件声明实现的生命周期API与执行命令的映射关系，提供能力发现端点供系统查询，收到请求后执行对应脚本并返回标准HTTP响应
- **FR-033**: 系统在执行版本升级（扩展或中间件）前，通过pre-upgrade生命周期API执行准备工作（如兼容性检查、数据备份）；升级后通过post-upgrade验证结果，失败时的回滚能力取决于扩展是否实现了rollback相关API
