...
- **FR-004**: 系统必须支持扩展版本升级和中间件版本升级，两者独立执行且均为单步升级；扩展版本升级要求目标扩展声明支持从当前版本升级上来，中间件版本升级遵循扩展声明的升级路径；升级失败均支持回滚
...
- **FR-030**: 平台必须提供通用生命周期API Server镜像，扩展基于此镜像制作专用镜像，只需提供配置文件声明API与脚本的映射关系，无需编写HTTP Server代码
- **FR-031**: 通用生命周期API Server必须通过配置文件声明实现的生命周期API与执行命令的映射关系，提供能力发现端点供系统查询，收到请求后执行对应脚本并返回标准HTTP响应
- **FR-032**: 系统在执行版本升级（扩展或中间件）前，通过pre-upgrade生命周期API执行准备工作（如兼容性检查、数据备份）；升级后通过post-upgrade验证结果，失败时支持回滚
